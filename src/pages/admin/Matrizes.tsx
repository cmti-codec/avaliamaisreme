...
const Matrizes = () => {
  ...
  const filteredMatrizes = useMemo(() => {
    ...
    return matrizes.filter(matriz => {
      const matchSearch = search === "" || matriz.codigo.toLowerCase().includes(search.toLowerCase()) || matriz.nome.toLowerCase().includes(search.toLowerCase()) || matriz.etapa_modalidade.toLowerCase().includes(search.toLowerCase());
      const matchStatus = statusFilter === "all" || statusFilter === "ativa" && matriz.ativa || statusFilter === "inativa" && !matriz.ativa;
      const matchEtapa = etapaFilter === "all" || matriz.etapa_modalidade === etapaFilter;
  ...
  return <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground max-w-3xl px-[2px]">Defina as matrizes curriculares para toda a rede. </p>
        </div>
        <div className="flex gap-2 shrink-0">
          {isMobile ? <>
              ...
              <Button onClick={() => setComponentesDialogOpen(true)} variant="outline" className="gap-2" size="lg">
                <Plus className="w-4 h-4" />
                Componentes
              </Button>
              <Button onClick={() => setCargasDialogOpen(true)} variant="outline" className="gap-2" size="lg">
        ...
        <CardContent className="p-4">
          <div className="flex gap-3">
            <div className="relative flex-1">
              ...
              <Input placeholder="Buscar matriz..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9" />
            </div>
            <Select value={etapaFilter} onValueChange={setEtapaFilter}>
              ...
              <SelectContent>
                ...
                {etapasDisponiveis.map(etapa => <SelectItem key={etapa} value={etapa}>
                    {etapa}
                  </SelectItem>)}
              </SelectContent>
            </Select>
      ...
      {/* Grid de Cards */}
      {filteredMatrizes.length === 0 ? <Card>
          ...
          {filteredMatrizes.map(matriz => <MatrizCard key={matriz.id} matriz={matriz} onView={handleView} onEdit={handleEdit} onDuplicate={handleDuplicate} onDelete={handleDelete} />)}
        </div>}

      {/* Dialogs */}
      <MatrizModal open={modalOpen} onClose={handleCloseModal} matrizId={matrizEditId} />

      <MatrizViewDialog open={viewDialogOpen} onClose={() => {
      setViewDialogOpen(false);
      setSelectedMatrizId(null);
    }} matrizId={selectedMatrizId} />

      <MatrizDeleteDialog open={deleteDialogOpen} onClose={() => {
      setDeleteDialogOpen(false);
      setSelectedMatrizId(null);
    }} matrizId={selectedMatrizId} />

      <ComponentesDialog open={componentesDialogOpen} onClose={() => setComponentesDialogOpen(false)} />

      <CargasHorariasDialog open={cargasDialogOpen} onClose={() => setCargasDialogOpen(false)} />
    </div>;
};
